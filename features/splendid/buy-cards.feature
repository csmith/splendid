Feature: players can buy cards
  ...
  Scenario: Alice tries to buy a card that doesn't exist
    When Alice buys the card 7/7/nonsense/99999
  ...
  Scenario: Alice buys card she can afford with normal tokens
    Given Alice had the following tokens:
      ...
      | diamond | 1      |
      | onyx    | 1      |
    When Alice buys the card 1/0/sapphire/00011
    Then Alice will have the following bonuses:
      | type     | amount |
      | sapphire | 1      |
  ...
  Scenario: Alice buys a card that gives points
    ...
    And the card in row 1 column 1 will be 3/3/diamond/07000
    And it will be Bob's turn
    And the following tokens will be available:
      | type     | amount |
      | diamond  | 4      |
