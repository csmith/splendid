Feature: players can receive nobles
  ...
  Background:
    ...
    And the following splendid nobles were available:
      | 03300 |
      | 00033 |
      | 22222 |

  Scenario: Alice buys a card which enables a noble visit
    Given Alice had the following splendid bonuses:
      | type     | amount |
      | sapphire | 3      |
      | ruby     | 2      |
    And Alice had the following splendid tokens:
      ...
      | ruby    | 1      |
      | onyx    | 1      |
    When Alice buys the splendid card 1/1/ruby/10101
  ...
  Scenario: Alice tries to receive a noble at the start of a turn
    Given Alice had the following splendid bonuses:
      ...
      | sapphire | 3      |
      | ruby     | 3      |
    When Alice receives the splendid noble 03300
  ...
  Scenario: Alice tries to receive an unknown noble
    ...
    When Alice receives the splendid noble 99999
  ...
  Scenario: Alice tries to receive an ineligible noble
    Given Alice had the following splendid bonuses:
      ...
      | ruby     | 3      |
    And the game phase was noble
    When Alice receives the splendid noble 22222
