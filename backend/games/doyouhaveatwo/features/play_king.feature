Feature: Play King
  ...
  Scenario: Player is able to perform action play_king when it's their turn and they have a King
    ...
    And player A sends action {"type": "dyhat:a:play_card_target_others", "player": "A", "card_name": "King", "target_player": "B"}
  ...
  Scenario: Player cannot perform action play_king when they don't have a King
    Given it is player B's turn
    When player B sends action {"type": "dyhat:a:play_card_target_others", "player": "B", "card_name": "King"}
    Then an error occurs
  ...
  Scenario: Cannot target yourself with King
    Given it is player A's turn
    When player A sends action {"type": "dyhat:a:play_card_target_others", "player": "A", "card_name": "King"}
    And player A sends action {"type": "dyhat:a:play_card_target_others", "player": "A", "card_name": "King", "target_player": "A"}
    Then an error occurs
  ...
  Scenario: Player can discard King when no valid targets
    ...
    And player B is eliminated
    And player C is protected
    When player A sends action {"type": "dyhat:a:discard_card", "player": "A", "card_name": "King"}
    Then no error occurs
    And player A should have 1 cards in discard pile