<script>
  ...
  $: hasFullSelection =
    _.sum(Object.values(selected)) === Math.min(availableGems, 3) || Object.values(selected).some((v) => v > 1);
  ...
  <div class="take" class:hidden={_.sum(Object.values(selected)) === 0}>
    ...
    <ul id="token-selection">
      {#each Object.entries(selected) as pair}
        <li>
          <GemCounter type={pair[0]} amount={pair[1]} interactive={pair[1] > 0} on:click={() => unselectGem(pair[0])} />
        </li>
      {/each}
    </ul>
